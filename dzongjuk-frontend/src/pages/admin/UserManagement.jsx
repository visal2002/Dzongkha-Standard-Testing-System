...
export default function UserManagement() {
  ...
  return <div className="space-y-6">
    ...
    <Modal isOpen={showCreate} onClose={() => setShowCreate(false)} title="Create New System User" size="md" footer={<><Button variant="ghost" onClick={() => setShowCreate(false)}>Cancel</Button><Button onClick={handleCreate}>Create User</Button></>}><div className="grid grid-cols-2 gap-4"><div className="col-span-2"><Input label="Full Name" required value={form.name} onChange={event => setForm(previous => ({ ...previous, name: event.target.value }))} /></div><Input label="Email Address" type="email" required value={form.email} onChange={event => setForm(previous => ({ ...previous, email: event.target.value }))} /><Input label="CID Number" required value={form.cid} onChange={event => setForm(previous => ({ ...previous, cid: event.target.value }))} /><div className="col-span-2"><Input label="Temporary Password" type="password" required value={form.password} onChange={event => setForm(previous => ({ ...previous, password: event.target.value }))} hint="At least 12 characters; the user should change it after first sign-in." /></div>{roleSelector}</div></Modal>
    <Modal isOpen={!!editingUser} onClose={() => setEditingUser(null)} title={`Edit User — ${editingUser?.name}`} size="md" footer={<><Button variant="ghost" onClick={() => setEditingUser(null)}>Cancel</Button><Button onClick={handleSaveEdit}>Save Changes</Button></>}><div className="grid grid-cols-2 gap-4"><div className="col-span-2"><Input label="Full Name" required value={form.name} onChange={event => setForm(previous => ({ ...previous, name: event.target.value }))} /></div><Input label="Email Address" type="email" required value={form.email} onChange={event => setForm(previous => ({ ...previous, email: event.target.value }))} /><Input label="CID Number" required value={form.cid} onChange={event => setForm(previous => ({ ...previous, cid: event.target.value }))} />{roleSelector}</div></Modal>
