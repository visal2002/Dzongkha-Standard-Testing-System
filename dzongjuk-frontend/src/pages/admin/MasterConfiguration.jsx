  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        action={<Button loading={loading} onClick={handleSave} icon={<Save size={14} />}>Save Changes</Button>}
      />
        ...
        <div className="mt-4">
          <TabPanel value="fees">
            <div className="bg-surface-card border border-surface-border rounded-xl p-5 grid grid-cols-2 gap-5">
              <Input
                ...
                onChange={e => setConfig(p => ({ ...p, registrationFee: +e.target.value }))}
                hint="Applied to all new registrations"
              />
              <Input
                label="Appeal Fee — Base (Nu.)"
                type="number"
                value={config.appealFee}
                onChange={e => setConfig(p => ({ ...p, appealFee: +e.target.value }))}
              />
              <Input
                label="Appeal Fee Per Skill (Nu.)"
                type="number"
                value={config.appealFeePerSkill}
                onChange={e => setConfig(p => ({ ...p, appealFeePerSkill: +e.target.value }))}
            ...
            <div className="bg-surface-card border border-surface-border rounded-xl p-5 space-y-5">
              <div className="grid grid-cols-2 gap-5">
                <Input
                  ...
                  type="number"
                  value={config.certificateValidity}
                  onChange={e => setConfig(p => ({ ...p, certificateValidity: +e.target.value }))}
                />
                <Select label="Paper Size" value={config.certificateTemplate.paperSize} onChange={e => setConfig(p => ({ ...p, certificateTemplate: { ...p.certificateTemplate, paperSize: e.target.value } }))}>
                  <option>A4</option>
                  <option>A3</option>
                  <option>Letter</option>
                </Select>
                <Select label="Orientation" value={config.certificateTemplate.orientation} onChange={e => setConfig(p => ({ ...p, certificateTemplate: { ...p.certificateTemplate, orientation: e.target.value } }))}>
                  <option>landscape</option>
                  <option>portrait</option>
                </Select>
                <Input
                  label="Certificate Title"
                  value={config.certificateTemplate.title}
                  onChange={e => setConfig(p => ({ ...p, certificateTemplate: { ...p.certificateTemplate, title: e.target.value } }))}
                />
                <Input
                  label="Authorized Signatory"
                  value={config.certificateTemplate.authorizedSignatureName}
                  onChange={e => setConfig(p => ({ ...p, certificateTemplate: { ...p.certificateTemplate, authorizedSignatureName: e.target.value } }))}
                />
              </div>
              <Textarea
                label="Declaration Statement"
                rows={3}
                value={config.certificateTemplate.declarationStatement}
                onChange={e => setConfig(p => ({ ...p, certificateTemplate: { ...p.certificateTemplate, declarationStatement: e.target.value } }))}
              />
            </div>
          </TabPanel>
          ...
          <TabPanel value="scoring">
            <div className="bg-surface-card border border-surface-border rounded-xl p-5 space-y-5">
              <div className="grid grid-cols-3 gap-5">
                <Input label="Max Band Score" type="number" value={config.maxBandScore} readOnly hint="Fixed at 9.0 per CEFR" />
                <Input label="Min Band Score" type="number" value={config.minBandScore} readOnly />
                <Input label="Score Step" type="number" value={config.bandScoreStep} step="0.5" hint="0.5 increment" readOnly />
              </div>
              <div>
                <p className="text-sm font-semibold text-text-primary mb-3">CEFR Band Levels</p>
                <div className="space-y-2">
                  {config.bandLevels.map(bl => (
          ...
          <TabPanel value="notifications">
            <div className="bg-surface-card border border-surface-border rounded-xl p-5 space-y-4">
              {Object.entries(config.notificationTemplates).map(([key, val]) => (
                <Textarea
                  key={key}
                  label={key.replace(/([A-Z])/g, ' $1').replace(/^./, s => s.toUpperCase())}
                  rows={2}
                  value={val}
                  onChange={e => setConfig(p => ({ ...p, notificationTemplates: { ...p.notificationTemplates, [key]: e.target.value } }))}
