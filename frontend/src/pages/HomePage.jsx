...
export default function HomePage() {
  ...
  const features = [
    { icon: Monitor, title: t('home.feature_registration_title'), desc: t('home.feature_registration_desc') },
    { icon: Award, title: t('home.feature_certificates_title'), desc: t('home.feature_certificates_desc') },
    { icon: ShieldCheck, title: t('home.feature_secure_title'), desc: t('home.feature_secure_desc') },
  ];
  ...
  return (
    ...
    >
      ...
      <header className="relative z-10 flex items-center justify-between px-10 py-5 max-w-[1920px] mx-auto w-full bg-[#0d1425]/60 backdrop-blur-md border-b border-white/5 shadow-md">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 font-medium tracking-widest uppercase mt-0.5">DSTS • DCDD</p>
          </div>
        </div>

        {/* Navigation */}
        <nav className="hidden md:flex items-center gap-9">
          <Link to="/" className="text-white font-medium hover:text-brand-gold-light transition-colors text-base drop-shadow-md">{t('home.nav_home')}</Link>
          <Link to="#" className="text-gray-300 font-medium hover:text-white transition-colors text-base drop-shadow-md">{t('home.nav_about')}</Link>
          <Link to="#" className="text-gray-300 font-medium hover:text-white transition-colors text-base drop-shadow-md">{t('home.nav_contact')}</Link>
        </nav>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1.2fr_0.8fr] gap-8 w-full">
            ...
            >
              <h1 className="text-6xl sm:text-[76px] font-black text-white leading-[1.1] mb-6 tracking-tight drop-shadow-lg">
                <span className="inline-block sm:whitespace-nowrap">{t('home.hero_title_line1')}</span><br />
                <span className="text-brand-gold-light">{t('home.hero_title_line2')}</span>
              </h1>
              ...
              <div className="flex items-center gap-5 flex-wrap">
                ...
                >
                  {t('home.cta_access_portal')} <ArrowRight size={20} />
                </Link>
                <Link
                  to="/ndi-login"
      ...
      <footer className="relative z-10 text-center py-4 text-[13px] text-white/90 font-medium flex flex-col gap-0.5 mt-6 border-t border-white/10 bg-[#0d1425]/40 backdrop-blur-sm">
        ...
        <p className="opacity-80">{t('home.footer_developed')}</p>
        <p className="opacity-80">{t('home.footer_department')}</p>
      </footer>
    </div>
  );
}
